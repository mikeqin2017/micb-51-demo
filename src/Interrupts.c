...
#define REFEREBCE  3300
uint32_t mV;
uint8_t ADC_complete;
SI_INTERRUPT(ADC0EOC_ISR, ADC0EOC_IRQn)
{
	static uint32_t accumulator = 0;     // Accumulator for averaging
	static uint16_t measurements = SAMPLE_NUMBER;// Measurement counter
	uint32_t result=0;
	...
	if(measurements==0)
	{
		measurements = SAMPLE_NUMBER;
		result = accumulator / SAMPLE_NUMBER;
		accumulator=0;
		mV = result * REFEREBCE / 1023;
		ADC_complete=1;
		//UART0_writeBuffer(buffer,5);
	}
}
